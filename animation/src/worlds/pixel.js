...
import { W, H, T, R, FLOOR, CAM_Z, CAM_CY, camX, clamp, lerp, invLerp, ease, rng, shake, ball2D, beat, TAU, BLOCK_HIT_H } from '../core.js';
import { makeCanvas } from '../fx.js';

const LW = 240, LH = 135, PX = 8, K = CAM_Z / PX;
const PAL = ['#000000', '#1d2b53', '#7e2553', '#008751', '#ab5236', '#5f574f', '#c2c3c7', '#fff1e8',
  '#ff004d', '#ffa300', '#ffec27', '#00e436', '#29adff', '#83769c', '#ff77a8', '#ffccaa'];
const HIT = beat(13), LAND = beat(14);
let lo, lg, big, bg2;
...
export default {
  ...
  draw(g, t) {
    const b = ball2D(t);
    const sk = shake(t, 3);
    const cx = Math.round(camX(t) * K) + Math.round(sk.x);
    const cy = CAM_CY * K + Math.round(sk.y);
    ...
    const hb = ball2D(HIT);
    const hitX = hb.x * K - cx + LW / 2;
    const blockBottom = Math.round((FLOOR - R - BLOCK_HIT_H - R) * K - cy + LH / 2);
    const dHit = t - HIT;
    ...
    for (let x = -sw; x <= sw; x++) if ((x + fl) % 2 === 0) px(lg, bxL + x, fl, 3, 1, 1);
    // Landing dust
    for (const ti of [T.PIXEL, LAND]) {
      const d = t - ti;
      if (d < 0 || d > 0.25) continue;
      const f = Math.floor(d / 0.0625);
      const ix = ball2D(ti).x * K - cx + LW / 2;
